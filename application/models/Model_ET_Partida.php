...
class Model_ET_Partida extends CI_Model
{
	...
	{
		$data=$this->db->query("select ETP.id_partida, ETP.id_meta, ETP.desc_partida, ETP.rendimiento, ETP.cantidad, ETDP.parcial, ETDP.precio_unitario, UM.id_unidad, UM.descripcion,ETP.numeracion from ET_PARTIDA as ETP inner join UNIDAD_MEDIDA as UM on ETP.id_unidad=UM.id_unidad left join ET_DETALLE_PARTIDA as ETDP on ETP.id_partida=ETDP.id_partida where id_meta='".$idMeta."'");
	...
	public function updateCambiosPartida($idPartida, $cantidad, $rendimiento)
	{
		$this->db->query("update ET_PARTIDA set cantidad=$cantidad, rendimiento='$rendimiento' where id_partida=".$idPartida);
